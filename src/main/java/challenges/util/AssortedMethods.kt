...
object AssortedMethods {
    ...
    fun randomBoolean(percentTrue: Int): Boolean {
    ...
    fun toFullBinaryString(a: Int): String {
        var a = a
        var s = ""
        for (i in 0..31) {
            val lsb = a and 1
    ...
    fun toBaseNString(a: Int, base: Int): String {
        var a = a
        var s = ""
        while (true) {
            val lastdigit = a % base
            s = lastdigit.toString() + s
            a /= base
            if (a == 0) break
        }
        return s
...
//    }
    ...
    fun listOfPointsToString(list: ArrayList<Point>): String {
    ...
    val wordListAsHashSet: HashSet<String>
        get() {
            val wordList = listOfWords
    ...
    fun getLongTextBlobAsStringList(): Array<String?>? {
        return getLongTextBlob().split(" ").toTypedArray()
    }

    fun getLongTextBlob(): String {
        val book =
    ...
    fun getLongSampleText(forceLower: Boolean): String {
        val text =
            """
            ...
            """.trimIndent()
        return if (forceLower) text.toLowerCase() else text
    }
